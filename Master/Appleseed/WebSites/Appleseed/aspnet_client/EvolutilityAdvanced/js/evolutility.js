...
Evol.UI = {
    ...
    // insert a dataSet into a Backbone collection  = HAPTIX 
...
Evol.ViewMany = Backbone.View.extend({
    ...
    // This method used for listing data 
    render: function () {
        var models = this.collection.models;
        Evol.UI.insertCollection(this.collection, jsmodels_data);
                ...
                if (collec) {
                    collec.create(this.getData(true), {
                        success: function (m) {
                            ...
                            //This ajax method is used to save and edit data into database
                            var collStr = JSON.stringify(collec);
                            collStr = collStr.replace(/"/g, "&quot;");
                            collStr = collStr.replace("[", "&sqbleft;");
                            collStr = collStr.replace("]", "&sqbright;");

                            collStr = collStr.replace("{", "&crbleft;");
                            collStr = collStr.replace("}", "&crbright;");

                            //&quot;
                            var dataParams = '{"moduleid":"' + getQSValueByName('moduleid') + '","data":"' + collStr + '"}';

                            $.ajax({
                                type: "POST",
                                url: "/DesktopModules/CoreModules/EvolutilityAdvanced.ModuleRenderer/EvolutilityAdvancedModuleRendererService.asmx/UpdateData",
                                data: dataParams,
                    ...
                    function () {
                        ...
                        delModel.destroy({
                            success: function () {
                                ...
                                // This ajax method is used to delete data from database
                                var collStr = JSON.stringify(collec);
                                collStr = collStr.replace(/"/g, "&quot;");
                                collStr = collStr.replace("[", "&sqbleft;");
                                collStr = collStr.replace("]", "&sqbright;");
...
Evol.Shell = Backbone.View.extend({
    ...
    createEntity: function ($v, uiModel, data, defaultView, options, cb) {
        ...
        var lc = new Backbone.LocalStorage('evol-' + uiModel.id + $.now());
